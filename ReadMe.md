<h1 align="center">Hi 👋, I'm Andrey</h1>
<h3 align="center">Fullstack Developer | TypeScript Enthusiast | Crafting modern web applications</h3>

---

### 🚀 About Me

💻 I build **fast**, **scalable** and **maintainable** web applications using modern technologies.  
⚙️ Tech Stack: **TypeScript, React, Next.js, Node.js, Express, MongoDB, PostgreSQL**  
🎮 I also work on projects in the **gaming industry** (Minecraft hosting, launchers, APIs).  
📈 I help businesses and communities create reliable infrastructure for their platforms.

---

### 🛠️ Skills & Technologies
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/-React-61DAFB?style=flat&logo=react&logoColor=white)
![Next.js](https://img.shields.io/badge/-Next.js-000?style=flat&logo=next.js)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/-Express-000?style=flat&logo=express&logoColor=white)
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/-PostgreSQL-336791?style=flat&logo=postgresql&logoColor=white)

---

### 🌍 What I'm working on:
- 💡 **Web apps** for clients from **Ukraine, Poland, Europe**
- 🎮 **Minecraft hosting platforms / launchers / APIs**
- 🔧 **Admin dashboards**, authentication systems, and SaaS MVPs  

---

### 📫 How to reach me:
<!-- [![Telegram Badge](https://img.shields.io/badge/-@YourUsername-blue?style=flat&logo=telegram&logoColor=white)](https://t.me/YourUsername)  
[![Discord Badge](https://img.shields.io/badge/-YourDiscordTag%230000-5865F2?style=flat&logo=discord&logoColor=white)](https://discordapp.com/users/YourDiscordID) -->

---

### 📊 GitHub Stats:
![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=Andrey2211553&show_icons=true&theme=radical&hide_rank=true)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Andrey2211553&layout=compact&theme=radical)

---

### ⚡ Fun Fact:
> I started my programming journey through game development and stayed for the joy of building real-world products.
